...
class Scanning_port:
	...
	def __init__(self, master):
		...
		ttk.Label(self.frame_header, text = "Nikto with Python Framework v 1.0", style = 'Header.TLabel').grid(row = 0, column = 1, padx = 5, pady = 5, sticky = 'sw')
		ttk.Label(self.frame_header, wraplength=295, text = "Nikto Scanning tool (cmd:Nikto -h target)").grid(row = 1, column = 1, padx = 5, sticky = 'sw')
		ttk.Label(self.frame_header, wraplength=295, text = "(Make sure, nikto is already installed on your machine)").grid(row = 2, column = 1, padx = 5, pady = 5, sticky = 'sw')
...
def index():
	...
	left10 = Label(labelframe, wraplength=300, background = "#e1d8b9", 
		text="Nikto is already licensed under GNU, I designed just GUI which runs only the nikto tools. visit nikto https://github.com/sullo/nikto", font = "Verdana 7").pack()
